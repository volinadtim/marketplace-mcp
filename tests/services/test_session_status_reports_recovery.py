...
from marketplace_mcp.services.session import session_status
...
def test_signed_in(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("marketplace_mcp.services.session.get_session", lambda: _FakeSession("44563249", backup=True))
...
def test_signed_out_with_a_backup_says_it_will_retry(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("marketplace_mcp.services.session.get_session", lambda: _FakeSession(None, backup=True))
    status = session_status()
...
def test_signed_out_without_a_backup_asks_for_a_login(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("marketplace_mcp.services.session.get_session", lambda: _FakeSession(None, backup=False))
    status = session_status()
    assert status.detail is not None
